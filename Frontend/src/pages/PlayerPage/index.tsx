...
export default function PlayerPage() {
  ...
  return (
    <div className="min-h-dvh bg-base-200">
      ...
      <RollHistoryToast />
      {tab === "combate" && player?.fightInfo?.battleStatus == "starting" && !isExecutingSkill && player?.fightInfo?.canRollInitiative && (
        <button
          className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-primary text-primary-content rounded-full w-32 h-32 flex flex-col items-center justify-center gap-1 animate-pulse shadow-lg cursor-pointer active:scale-95 transition-transform"
        ...
        >
          ...
          <span className="text-xs font-bold leading-tight text-center">{t("combat.rollInitiative")}</span>
        </button>
      )}
